...
void AddMemBuf::visit(const Nest& nest) {
    sched_ = Scheduler{nest};
    add_mem_to_lams(root(), root());
}

void AddMemBuf::set_mem(Lam* place, const Def* mem) {
    val2mem_[place] = mem;
    if (auto it = mem_rewritten_.find(place); it != mem_rewritten_.end()) val2mem_[it->second] = mem;
}

const Def* AddMemBuf::mem_for_lam(Lam* lam) const {
...
const Def* AddMemBuf::rewrite_type(const Def* type) {
...
const Def* AddMemBuf::rewrite_pi(const Pi* pi) {
...
const Def* AddMemBuf::add_mem_to_lams(Lam* curr_lam, const Def* def) {
