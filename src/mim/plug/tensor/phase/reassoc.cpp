...
/// `dot_schedule` runs @p z as the vector loop, so a literal @p z pads to a whole number of @p vec lanes;
/// a symbolic one is assumed to vectorize perfectly.
Poly mul_cost(const Def* x, const Def* y, const Def* z, u64 vec) {
    auto coeff = 1_u64;
    auto syms  = DefVec();
...
Poly cost_of(fe::View<Split> splits, Defs dims, u64 vec) {
    auto poly = Poly();
    for (auto [i, s, j] : splits)
        poly.add(mul_cost(dims[i], dims[s + 1], dims[j + 1], vec));
    return poly;
}
...
fe::Vector<Splits> pareto(fe::View<Splits> cands, Defs dims, u64 vec) {
    auto keep  = fe::Vector<Splits>();
    auto costs = fe::Vector<Poly>();

    for (const auto& cand : cands) {
        auto cost = cost_of(cand, dims, vec);
...
std::optional<std::pair<fe::Vector<u64>, Poly>> matrix_chain_order(Defs dims, u64 vec) {
    auto n     = dims.size() - 1;
    ...
    for (auto len = 2_u64; len <= n; ++len) {
        for (auto i = 0_u64; i + len <= n; ++i) {
            ...
            for (auto s = i; s != j; ++s) {
                auto c = mul_cost(dims[i], dims[s + 1], dims[j + 1], vec);
                c.add(cost[i * n + s]);
                c.add(cost[(s + 1) * n + j]);
...
const Def* Reassoc::cost_expr(Defs dims, const Splits& splits) {
    ...
    // The vector loop pads as in mul_cost, so the run-time tournament ranks by the same cost model.
    auto vec_dim = [&](const Def* d) -> const Def* {
        if (auto l = Lit::isa<u64>(d)) return w.lit_nat(lanes(*l, vec_));
        return rewrite(d);
    };

    for (auto [i, s, j] : splits) {
        auto p = w.app(w.annex(core::nat::mul), {rewrite(dims[i]), rewrite(dims[s + 1])});
        p      = w.app(w.annex(core::nat::mul), {p, vec_dim(dims[j + 1])});
...
const Def* Reassoc::reassoc(const App* app) {
    ...
    auto orig_cost = cost_of(orig, dims, vec_);

    if (n <= max_dispatch_) {
        auto cands = pareto(bracketings(0, n - 1), dims, vec_);
        if (cands.size() != 1) {
        ...
        auto cost = cost_of(cands.front(), dims, vec_);
    ...
    auto order = matrix_chain_order(dims, vec_);
