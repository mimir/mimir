...
void* open(const char* file) {
    ...
    if (void* handle = dlopen(file, RTLD_NOW))
        ...
        fe::throwf("could not load plugin `{}` due to error `{}`", file, err);
    else
        fe::throwf("could not load plugin `{}`", file);
#endif
}
...
void* get(void* handle, const char* symbol) {
    ...
    if (auto err = dlerror())
        fe::throwf("could not find symbol `{}` in plugin due to error `{}`", symbol, err);
    else
        return addr;
#endif
}
...
#else
    if (auto err = dlclose(handle)) fe::throwf("`dlclose()` failed with error code `{}`", err);
#endif
}
