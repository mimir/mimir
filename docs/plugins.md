...
The function `mim_get_plugin` is the single entry point [`Driver`](@ref mim::Driver) looks up when a `plugin demo;` directive or `-p demo` loads the plugin - via `dlopen`/`dlsym`, or from a table of the plugins linked in when `MIM_STATIC_PLUGINS` is set.
