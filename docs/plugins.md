...
It returns a [`mim::Plugin`](@ref mim::Plugin) record: the plugin's name, the `MIM_VERSION` it was built against (checked against the loading `mim` binary), the `register_normalizers` function that `normalizers.cpp` defined via `MIM_demo_NORMALIZER_IMPL`, an optional backend-registration callback (`nullptr` here, since `demo` provides no backend), and the [`-X` arguments](@ref mim::PluginArg) it understands (none here).
