...
private:
    ...
    void recover(fe::Cite ctxt) { Super::recover(Tok::is_delim_r, ctxt); }
    ///@}

    /// @name parse exprs
    ///@{
    Ptr<Expr> parse_expr(fe::Cite ctxt, Prec = Prec::Bot);

    /// As above but builds @p ctxt via fe::format_cite.
    template<class... Args>
    Ptr<Expr> parse_expr(fe::cite_string<Args...> fmt, Args&&... args) {
        return parse_expr(fe::format_cite(fmt, std::forward<Args>(args)...));
    }

    /// As above but with @p prec.
    template<class... Args>
    Ptr<Expr> parse_expr(Prec prec, fe::cite_string<Args...> fmt, Args&&... args) {
    ...
    Ptr<Ptrn> parse_ptrn(PtrnStyle, fe::Cite ctxt, Prec = Prec::Bot);

    /// As above but builds @p ctxt via fe::format_cite.
    template<class... Args>
    Ptr<Ptrn> parse_ptrn(PtrnStyle style, Prec prec, fe::cite_string<Args...> fmt, Args&&... args) {
        return parse_ptrn(style, fe::format_cite(fmt, std::forward<Args>(args)...), prec);
    }
    Ptr<Ptrn> parse_ptrn_(PtrnStyle, fe::Cite ctxt, Prec = Prec::Bot);
