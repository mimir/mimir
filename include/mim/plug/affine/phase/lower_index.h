...
///
/// The `%affine.map f idxs` bridge lowers to: widen each runtime `idxs#i : Idx (sin#i)` to `Idx 0` via `%core.conv.u`,
/// apply the (now core-arithmetic) function `f`, and narrow each result back to its target `Idx (sout#j)` via
/// `%core.conv.u`.
///
/// The division-based semiops (`ceildiv`, `floordiv`, `mod`) are not yet lowered (they require `%core.div`, which
/// threads a `%mem.M`); they currently raise an error.
