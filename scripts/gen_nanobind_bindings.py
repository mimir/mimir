...
def _qualified_name(decl) -> str:
    ...
    in `__fs`) and `fe::Sets::Set` recognisable everywhere.
    """
    parts = []
    cursor = decl
...
_MIM_SET = "fe::Sets::Set"  # `Vars`/`Muts`: a forward range of `D*`
...
def _is_bound_class(decl) -> bool:
    ...
    (`World::State`, `Sets<D, K>::Set`) or a template specialization never is.
    """
...
def _range_value_type(t) -> tuple[str, set] | None:
    ...
    `Sets<D, K>::Set` — whose iterator hands out `D*` — yields `D*`.
    """
    canon = t.get_canonical()
    ...
    if name in _MIM_RANGES:
        ...
        # `Set` is nested in the `Sets<D, K, N>` specialization, which is where the
