...
def _is_bound_class(decl) -> bool:
    ...
    (`World::State`, `Patricia<D, KT>::Set`) or a template specialization never is.
    """
...
def _range_value_type(t) -> tuple[str, set] | None:
    ...
    `Patricia<D, KT>::Set` — whose iterator hands out `D*` — yields `D*`.
    """
    canon = t.get_canonical()
    ...
    # `Patricia` hands out pointers to its element type, the other two store the
    # element type itself.
    elem, elem_is_pointee = None, False
    if name in _MIM_RANGES:
        elem = canon.get_template_argument_type(0)
    elif name == _MIM_SET:
        # `Set` is nested in the `Patricia<D, KT, K, N>` specialization, which is where the
