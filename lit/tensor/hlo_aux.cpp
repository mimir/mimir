...
float* main_hlo(float* arg0, float* arg1, float* arg2, float* arg3, float* arg4, float* arg6, float* arg7,
                float* arg5);
}

static void initialize_array(std::vector<float>& arr) {
    for (size_t i = 0; i < arr.size(); ++i) arr[i] = static_cast<float>(i % 100) / 100.0f;
}
...
int main() {
    ...
    std::vector<float> arg0(IN * H), arg1(H), arg2(H * H), arg3(H), arg4(H * OUT), arg5(OUT), arg6(B * IN),
        arg7(B * OUT);
    for (auto* a : {&arg0, &arg1, &arg2, &arg3, &arg4, &arg5, &arg6, &arg7}) initialize_array(*a);

    float* ret = main_hlo(arg0.data(), arg1.data(), arg2.data(), arg3.data(), arg4.data(), arg6.data(),
                          arg7.data(), arg5.data());

    // Reference forward pass.
    std::vector<float> mm0(B * H), h0(B * H), mm1(B * H), h1(B * H), mm2(B * OUT);
    matmul(arg6.data(), arg0.data(), mm0.data(), B, IN, H);
    for (size_t i = 0; i < B * H; ++i) h0[i] = std::tanh(mm0[i] + arg1[i % H]);
    matmul(h0.data(), arg2.data(), mm1.data(), B, H, H);
    for (size_t i = 0; i < B * H; ++i) h1[i] = std::tanh(mm1[i] + arg3[i % H]);
    matmul(h1.data(), arg4.data(), mm2.data(), B, H, OUT);

    std::vector<float> ref(B);
    for (size_t i = 0; i < B; ++i) {
        float m = -INFINITY;
        for (size_t j = 0; j < OUT; ++j) m = std::max(m, mm2[i * OUT + j] + arg5[j]);
        ref[i] = m;
    }
    ...
    for (size_t i = 0; i < B; ++i) {
        ...
        if (rel > 1e-3f) {
            if (failures < 8)
                std::cout << "mismatch at " << i << ": ref " << ref[i] << " got " << ret[i] << " (rel "
                          << rel << ")\n";
            ++failures;
        }
    }
