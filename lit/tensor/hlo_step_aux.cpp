...
extern "C" {
float main_hlo(float* arg0, float* arg1, float* arg2, float* arg3, float* arg4, float* arg5, float* arg6,
               float* arg7);
}
...
int main() {
    ...
    for (size_t i = 0; i < W1.size(); ++i) W1[i] = 0.05f * (static_cast<float>(i % 89) / 89.0f - 0.5f);
    for (size_t i = 0; i < W2.size(); ++i) W2[i] = 0.05f * (static_cast<float>(i % 83) / 83.0f - 0.5f);
    for (size_t i = 0; i < W3.size(); ++i) W3[i] = 0.1f * (static_cast<float>(i % 79) / 79.0f - 0.5f);
    for (size_t i = 0; i < b1.size(); ++i) b1[i] = 0.01f * (static_cast<float>(i % 7) - 3.0f);
    for (size_t i = 0; i < b2.size(); ++i) b2[i] = 0.01f * (static_cast<float>(i % 5) - 2.0f);
    for (size_t i = 0; i < b3.size(); ++i) b3[i] = 0.01f * (static_cast<float>(i % 3) - 1.0f);
    for (size_t i = 0; i < X.size(); ++i) X[i] = 0.05f * (static_cast<float>(i % 97) / 97.0f);
    for (size_t i = 0; i < B; ++i)
        for (size_t j = 0; j < O; ++j) Y[i * O + j] = (j == i % O) ? 1.0f : 0.0f;

    float got = main_hlo(W1.data(), b1.data(), W2.data(), b2.data(), W3.data(), b3.data(), X.data(),
                         Y.data());
    ...
    Vec h1(B * H), one_m_h1(B * H), h2(B * H), one_m_h2(B * H), logits(B * O);
    {
        ...
        for (size_t i = 0; i < B * O; ++i) logits[i] += b3[i % O];
    }
    ...
    for (size_t i = 0; i < B; ++i) {
        float rmax = -INFINITY;
        for (size_t j = 0; j < O; ++j) rmax = std::max(rmax, logits[i * O + j]);
    ...
    for (size_t i = 0; i < B; ++i)
        for (size_t k = 0; k < H; ++k) {
            float s = 0.0f;
            for (size_t j = 0; j < O; ++j) s += dlog[i * O + j] * W3[k * O + j];
    ...
    for (size_t i = 0; i < B; ++i)
        for (size_t k = 0; k < H; ++k) {
            float s = 0.0f;
            for (size_t l = 0; l < H; ++l) s += v59[i * H + l] * W2[k * H + l];
    ...
    auto colsum = [](const Vec& m, size_t stride, size_t col) {
        float s = 0.0f;
        for (size_t i = 0; i < B; ++i) s += m[i * stride + col];
        return s;
    };
    float dW1_35 = 0.0f, dW2_11_13 = 0.0f, dW3_19_2 = 0.0f;
    for (size_t i = 0; i < B; ++i) dW1_35 += v68[i * H + 5] * X[i * I + 3];      // v_73[3][5] = v_72[5][3]
    for (size_t i = 0; i < B; ++i) dW2_11_13 += v59[i * H + 13] * h1[i * H + 11]; // v_64[11][13] = v_63[13][11]
    for (size_t i = 0; i < B; ++i) dW3_19_2 += dlog[i * O + 2] * h2[i * H + 19];  // v_55[19][2] = v_54[2][19]
    ...
    float g[6] = {delta(W1[3 * H + 5], dW1_35),      delta(b1[7], db1_7), delta(W2[11 * H + 13], dW2_11_13),
                  delta(b2[17], db2_17),             delta(W3[19 * O + 2], dW3_19_2),
                  delta(b3[3], db3_3)};
    float ref  = g[0] + (g[1] + (g[2] + (g[3] + (g[4] + g[5])))); // matches the add nesting in hlo.mim

    float scale = 0.0f;
    for (float gi : g) scale += std::abs(gi);
    float diff = std::abs(got - ref);
    std::printf("got=%.9g ref=%.9g diff=%.3g scale=%.3g\n", got, ref, diff, scale);
    for (int i = 0; i < 6; ++i) std::printf("  g%d=%.9g\n", i, g[i]);
